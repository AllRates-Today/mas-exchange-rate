# Monetary Authority of Singapore Exchange Rates API — mas-exchange-rate

[![npm version](https://img.shields.io/npm/v/mas-exchange-rate.svg)](https://www.npmjs.com/package/mas-exchange-rate)
...
- 🏛️ **Official published rates** — Monetary Authority of Singapore's own table, with the publisher's own `rate_date` on every response
- 📅 **History back to 1988** — point-in-time tables and daily series for any past date
- 🔀 **Published vs derived, always flagged** — computed inverse/cross pairs carry `derived: true`, never mixed with official prints
- ⚡ **Zero dependencies** — pure ESM + CJS over global `fetch`; Node 18+, Bun, Deno, and edge runtimes
- 🔷 **Type-safe** — full TypeScript definitions shipped with the package
- 🧾 **Compliance-grade metadata** — `rate_type`, publication date, and source disclaimer on every response

> **Official rate, not mid-market:** every value here is a number Monetary Authority of Singapore itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Monetary Authority of Singapore table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/mas?source=USD&target=SGD"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/mas').then((x) => x.json());
...
```bash
yarn add mas-exchange-rate
```

```bash
pnpm add mas-exchange-rate
```

Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/mas-exchange-rate`](https://www.npmjs.com/package/@allratestoday/mas-exchange-rate) — same code, same versions.
...
console.log(pair.rate, pair.rate_date); // the official Monetary Authority of Singapore rate, on the central bank's own date
```
...
```js
import { getLatestRates } from 'mas-exchange-rate';
...
Paid plans. The official table for any date since 1988 — weekends and holidays return the most recent published date, flagged via `published_on_requested_date`, which is exactly the in-force rate a filing needs.

```js
import { getRatesForDate } from 'mas-exchange-rate';

const day = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...' });
// Optionally narrow to one pair:
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'USD', target: 'SGD' });
```
...
- Publisher's own page: [S$ exchange rates](https://www.mas.gov.sg/statistics/exchange-rates) · [www.mas.gov.sg](https://www.mas.gov.sg)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Monetary Authority of Singapore rates page](https://allratestoday.com/central-bank-rates-api/mas/)
...
- Monetary Authority of Singapore quotes **SGD per 1 unit of foreign currency** (e.g. `base: "USD", quote: "SGD"` means SGD per one US dollar).
...
Loading the official Monetary Authority of Singapore rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/mas/latest?format=xml&api_key=art_live_..."
```
...
If Monetary Authority of Singapore does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via SGD from two published rates |
...
| `404` | Pair or date range not covered by Monetary Authority of Singapore |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'mas-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('mas-exchange-rate');

getRate('USD', 'SGD', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- [Monetary Authority of Singapore rates page](https://allratestoday.com/central-bank-rates-api/mas/) — live table, publication cadence, FAQ
- [All central bank sources](https://allratestoday.com/central-bank-rates-api/)
- [Package docs on the site](https://allratestoday.com/docs/sdk/mas-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
...
- [GitHub](https://github.com/AllRates-Today/mas-exchange-rate)
